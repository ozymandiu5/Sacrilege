execute if entity @s[tag=sculk] run data modify entity @e[tag=altarText,limit=1,sort=nearest] text set value '[{"text":"Are you absolutely sure?","color":"dark_aqua","bold":true,"italic":false},{"text":"\\nThere is no going back.","color":"red","italic":true},{"text":"\\n(right click again)"}]'
